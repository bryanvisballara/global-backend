...
function renderSearchResults(matches) {
  ...
  trackingSearchResults.innerHTML = matches
    .map((order) => {
      ...
      const detailUrl = `/app/admin-tracking.html?orderId=${encodeURIComponent(orderId)}&tracking=${encodeURIComponent(trackingValue)}&vin=${encodeURIComponent(vinValue)}&internal=${encodeURIComponent(internalValue)}`;

      return `
        <a class="tracking-order-result ${orderId === selectedOrderId ? "selected" : ""}" href="${detailUrl}" data-order-link="true" data-order-id="${escapeHtml(orderId)}">
          <div>
            <strong>${escapeHtml(formatOrderLabel(order))}</strong>
            <p>Tracking ${escapeHtml(order.trackingNumber)} · VIN ${escapeHtml(order.vehicle?.vin || "Sin VIN")}</p>
            <p>${escapeHtml(getInternalIdentifier(order) || "Sin identificador interno")}</p>
          </div>
        </a>
      `;
    })
...
function renderOrderSummary(order) {
  ...
  trackingOrderSummary.innerHTML = `
    <div class="tracking-card-header">
      <strong>${escapeHtml(formatOrderLabel(order))}</strong>
      <p>${escapeHtml(order.client?.name || "Cliente sin asignar")} · Tracking ${escapeHtml(order.trackingNumber)}</p>
      <p>VIN ${escapeHtml(order.vehicle?.vin || "Sin VIN")} · Llegada estimada ${escapeHtml(formatDateLabel(order.expectedArrivalDate))}</p>
      <p>${escapeHtml(getInternalIdentifier(order) ? `Identificador interno ${getInternalIdentifier(order)}` : "Sin identificador interno")}</p>
    </div>
  `;
}
      ...
      ${mediaItems.map((item) => {
        ...
        return `
          <div class="tracking-saved-file-row">
            ...
            <button type="button" data-remove-media-state="${escapeHtml(stateKey)}" data-remove-media-index="${item.__index}">x</button>
          </div>
        `;
      }).join("")}
    </div>
    ...
    <div class="tracking-saved-files-list">
      ${files.map((file, index) => `
        <div class="tracking-saved-file-row pending">
          ...
          <button type="button" data-remove-pending-state="${escapeHtml(stateKey)}" data-remove-pending-category="${escapeHtml(category)}" data-remove-pending-index="${index}">x</button>
        </div>
      `).join("")}
    </div>
  `;
...
function renderVisibilityButton(stateKey, nextVisible, isCurrentlyVisible) {
  return `
    <button
      type="button"
...
function buildRecentEvents(order) {
  ...
  return states
    .map((state, index) => {
      const hasData = state.confirmed || state.updatedAt || state.confirmedAt || state.notes || (state.media || []).length;
      ...
      const eventDate = state.updatedAt || state.confirmedAt || null;
      ...
      return {
        ...
        title: state.confirmed
          ? `Cambio de etapa a ${stateCode} - ${state.label}`
          : `Actualizacion en ${stateCode} - ${state.label}`,
        location: "-",
        description: state.notes || ((state.media || []).length ? `${(state.media || []).length} adjunto(s) cargados.` : "-"),
...
function buildDocumentEntries(order) {
  const states = order?.trackingSteps || [];

  return states
    .flatMap((state, index) => {
      const stateCode = getStateCode(index);
      const statusLabel = `${stateCode} ${state.label}`;

      return (state.media || []).map((item, mediaIndex) => ({
        id: `${state.key}-${mediaIndex}`,
        stateKey: state.key,
        typeLabel: resolveDocumentTypeLabel(item),
        fileName: item.name || item.caption || `Archivo ${mediaIndex + 1}`,
        fileUrl: item.url,
        note: item.caption || state.notes || "-",
        date: state.updatedAt || state.confirmedAt || null,
        status: statusLabel,
        clientVisible: Boolean(state.clientVisible),
      }));
    })
...
async function updateStateClientVisibility(stateKey, nextVisible) {
  ...
  adminSetFeedback(trackingFeedback, nextVisible ? "Estado visible para cliente." : "Estado oculto para cliente.", "success");
}

function renderTrackingOverview(order) {
  const states = order?.trackingSteps || adminTrackingTemplates;
  const events = buildRecentEvents(order);
  const documents = buildDocumentEntries(order);
            ...
            .map((state, index) => {
              ...
              return `
                <button
                  ...
                  data-edit-state-key="${escapeHtml(state.key)}"
                >
                ...
                ? events
                    .map((event) => `
                      <tr>
                        ...
                        <td data-label="Etapa">${escapeHtml(event.stateCode)}</td>
                        <td data-label="Titulo">${escapeHtml(event.title)}</td>
                        <td data-label="Ubicacion">${escapeHtml(event.location)}</td>
                        <td data-label="Descripcion">${escapeHtml(event.description)}</td>
                        <td data-label="Cliente">${renderVisibilityButton(event.stateKey, !event.clientVisible, event.clientVisible)}</td>
                      </tr>
                    `)
                    .join("")
                : '<tr><td colspan="6"><div class="empty-state">No hay eventos registrados para esta orden.</div></td></tr>'}
            </tbody>
          </table>
        </div>
      </article>
                ...
                ? documents
                    .map((document) => `
                      <tr>
                        ...
                        <td data-label="Archivo"><a class="tracking-document-link" href="${escapeHtml(document.fileUrl)}" target="_blank" rel="noreferrer">${escapeHtml(document.fileName)}</a></td>
                        <td data-label="Estado">${escapeHtml(document.status)}</td>
                        <td data-label="Nota">${escapeHtml(document.note)}</td>
                        <td data-label="Fecha">${escapeHtml(formatDateTimeLabel(document.date))}</td>
                        <td data-label="Cliente">${renderVisibilityButton(document.stateKey, !document.clientVisible, document.clientVisible)}</td>
                      </tr>
                    `)
                    .join("")
                : '<tr><td colspan="6"><div class="empty-state">No hay documentos asociados a esta orden.</div></td></tr>'}
            </tbody>
          </table>
        </div>
      </article>
...
function renderStates() {
  ...
  trackingStatesList.innerHTML = states
    .map((state, index) => {
      ...
      return `
        <article class="tracking-state-card ${state.confirmed ? "is-confirmed" : ""} ${isExpanded ? "is-open" : ""}" data-state-card="${escapeHtml(state.key)}">
          <div class="tracking-state-header">
            ...
            <button class="tracking-state-edit-button ${state.confirmed ? "is-confirmed" : ""}" type="button" data-edit-state-key="${escapeHtml(state.key)}">${state.confirmed ? "Confirmado" : "Editar"}</button>
          </div>
          <div class="tracking-state-body" ${isExpanded ? "" : "hidden"}>
            <div class="tracking-state-switches">
              <label class="dashboard-checkbox"><input type="checkbox" data-field="confirmed" ${state.confirmed ? "checked" : ""} /> Activar estado</label>
              <label class="dashboard-checkbox"><input type="checkbox" data-field="hiddenFromClient" ${state.clientVisible ? "" : "checked"} /> Ocultar al cliente</label>
            </div>
            <div class="tracking-state-upload-grid two-up">
              <label>
                <span>Subir documento (PDF)</span>
                <input type="file" data-field="documentFile" data-state-key="${escapeHtml(state.key)}" accept=".pdf,application/pdf" />
                ${renderFileQueueBlocks(mediaBuckets.document, pendingMedia.document, state.key, "document")}
              </label>
              <label>
                <span>Subir foto unica</span>
                <input type="file" data-field="singlePhotoFile" data-state-key="${escapeHtml(state.key)}" accept="image/*" />
            ...
            <div class="tracking-state-upload-grid two-up">
              <label>
                ...
                <input type="file" data-field="carouselPhotoFiles" data-state-key="${escapeHtml(state.key)}" multiple accept="image/*" />
                ${renderFileQueueBlocks(mediaBuckets.photoCarousel, pendingMedia.photoCarousel, state.key, "photoCarousel")}
              </label>
              <label>
                <span>Video</span>
                <select data-field="videoSourceMethod" data-state-key="${escapeHtml(state.key)}">
                ...
                ${videoMethod === "link"
                  ? `<input type="url" data-field="videoLink" data-state-key="${escapeHtml(state.key)}" value="${escapeHtml(videoLink)}" placeholder="https://www.youtube.com/watch?v=..." />`
                  : `<input type="file" data-field="videoFile" data-state-key="${escapeHtml(state.key)}" accept="video/*" />`}
                ${videoMethod === "file" ? renderFileQueueBlocks(mediaBuckets.video, pendingMedia.video, state.key, "video") : ""}
              </label>
            </div>
            <label>
              <span>Notas</span>
              <textarea rows="4" data-field="notes" placeholder="Describe lo ocurrido en este estado">${escapeHtml(state.notes || "")}</textarea>
            </label>
            <div class="tracking-state-actions">
              <button class="primary-button" type="button" data-save-state-key="${escapeHtml(state.key)}">Guardar estado</button>
...
async function saveState(stateKey) {
  ...
  const hiddenFromClientField = stateCard.querySelector('[data-field="hiddenFromClient"]');
...
trackingPreview.addEventListener("click", (event) => {
  ...
  if (editStateButton) {
    expandedStateKey = String(editStateButton.dataset.editStateKey || "");
  ...
  const visibilityButton = event.target.closest("[data-toggle-client-visible][data-state-key][data-next-visible]");
  ...
  updateStateClientVisibility(stateKey, nextVisible).catch((error) => {
...
trackingRoot.addEventListener("change", (event) => {
  ...
  const confirmedField = event.target.closest('[data-field="confirmed"]');

  if (!confirmedField) {
    return;
  }

  const stateCard = confirmedField.closest("[data-state-card]");
